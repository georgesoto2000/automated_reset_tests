...
class capturecard(VideoCapture): #capturecard class
    ...
    def capture(self): #Takes a capture through USB capture card
        self.set(cv.CAP_PROP_FRAME_WIDTH, 1920) #Takes capture in 1080p
        self.set(cv.CAP_PROP_FRAME_HEIGHT, 1080) 
        return_value, image = self.read() #Opens capture card in read mode
