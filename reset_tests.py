...
def DEFAULT_TRANSPONDER(box,card): #Performs Default Transponder test
...
def FACTORY_RESET(box,card): 
    ...
    time.sleep(5)
    if not card.test("satellite"): #LNB type should revert to wideband, so there shouldn't be a 'No satellite signal' OSM
    ...
    if card.test("downloads.") and box.get_audio()==default_audio and box.get_resolution()==default_resolution: #checks audio settings and resolutions revert to default, and that recordings tray is empty checking for "No recordings or downloads." OSM to appear
...
def NVRAM_RESET(box,card): 
    ...
    box.press("backup home")
    time.sleep(5)
    if not card.test("satellite"): #check for 'No satellite signa' message, should not appear as LNB should have reverted to wideband
    ...
    if ((not card.test("downloads.")) and box.get_audio()==default_audio and box.get_resolution()==default_resolution): #recordings menu shouldn't be empty (so no "No recordings or downloads." OSM), AV settings should revert to defaults
...
def HARD_DISK_RESET(box,crd): 
    ...
    box.press("backup home")
    time.sleep(5)
    if card.test("satellite"): #LNB should stay set SCR to SCR so 'No satellite signal' OSM should appear
    ...
    box.reboot()
    box.recordings_menu()
    if (card.test("You have no recordings or downloads.") and box.get_audio()==audio and box.get_resolution()==res): #recordings menu should be empty so "no recordings or downloads." OSM, AV settings should be the same as before reset
